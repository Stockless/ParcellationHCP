...
void write_parcels(const std::string &path, const std::string &pathBundles,
	...
	std::cout<<path<<" "<<pathBundles<<std::endl;
	...
	for(uint16_t i=0; i<Tri.size(); i++) {
		std::string bundle = "tri_"+std::to_string(i)+".txt";
		if(Tri[i].size() == 0) continue;
		std::ofstream file(path + bundle, std::ios::out);

		std::cout << path + bundle << std::endl;

		if(file.is_open()) {

			const uint32_t len_Tri = Tri[i].size();
			file<<len_Tri<<std::endl; // número de triágulos iniciales
