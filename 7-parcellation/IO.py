...
def create_atlas_dirs(path,trac):
    ...
    os.mkdir(cc_path)
    return atlas_path,trac_path,connect_path,cc_path
...
def read_intersection_large_fascicle(intersection,triangles,names,bundle,thr=100):
    """
    ...
    """
    conn_dict = {}
    bundle_names = utils.get_bundle_names(bundle)
...
def read_intersection_file(infile,subject_name,name,parcel_names,triangles,anatomic_parcels):
    with open(infile) as f:
        ...
        if (content[0] != '0\n'):
            ...
            if parcel1_name in parcel_names:
            ...
            for i in range(len(fibers)):
                ...
                parcel1.fibers.add(fiber) 
                parcel2.fibers.add(fiber)
...
def write_hparcels_triangles(aparcels,atlas_path,hemi):
    output_path = atlas_path+"/hard_parcels/"+hemi
...
def write_parcels_cc(cc_parcels,cc_path,hemi):
    ...
    for k, Tris in cc_parcels.items():
        bt.write_parcels(cc_path+"/"+hemi+"/parcel_"+str(k)+".parcelsdata",[tri.index for tri in Tris])
        
def write_cc(cc_parcels,cc_path,hemi):
    if not os.path.exists(cc_path+"/"+hemi):
        os.makedirs(cc_path+"/"+hemi)
    for k, Tris in cc_parcels.items():
