...
def fusion(aparcels,anatomic_parcel,sub_parcel_list,parcel_names):
    while(len(sub_parcel_list)!=1):
        ...
        new_fibers = sub_parcel1.fibers.union(sub_parcel2.fibers)
...
def get_hard_parcels(aparcels):
    ...
    for ap in aparcels:
        ...
        for k,sp in ap.sub_parcels.items():
            for tri in sp.triangles:
                ...
                if selected_parcel != -1:
                    # print(selected_parcel,tri.prob_map[selected_parcel])
        ...
        for label, triangles in hparcels_map.items():
            ...
            ap.add_hparcel_triangles(label,triangles)
...
def get_PCC(aparcels,triangles):
    ...
    for ap in aparcels:
        ...
        for hp in ap.hard_parcels:
            parcel_cc[hp.label] = PCC(hp,triangles_arr)
...
def PCC(HP,triangles):
    Tri = np.array(list(HP.triangles))
    if len(Tri) == 0:
        return Tri
    
    edges_poly = []
    for tri in Tri:
        edges_poly.append([tri.v1.index,tri.v2.index])
        edges_poly.append([tri.v1.index,tri.v3.index])
        edges_poly.append([tri.v2.index,tri.v3.index])
    ...
    # indices = [ind.index for ind in Tri]
    ind = [np.where(triangles[HP.triangles_ind]==region_cc)[0] for region_cc in regions_cc];
    ind = np.unique(np.concatenate(ind));

    return Tri[ind]

def get_PCC_2(aparcels,triangles):
    parcel_cc = {}
    triangles_arr = np.asarray([[tri.v1.index,tri.v2.index,tri.v3.index] for tri in triangles])
    t1 = time.time()
    n = 0
    for ap in aparcels:
        ...
        for k,tris in ap.hparcels.items():
            parcel_cc[k] = PCC_2(tris,triangles_arr)
...
def main():
    ...
    start = time.time()
    atlas_path,trac_path,connect_path,cc_path = IO.create_atlas_dirs(args.output_dir,args.traceability)
    ...
    if trac == "y":
        ...
        update_fibers_map(Ranatomic_parcels)
    ...
    t1 = time.time()
    Lanatomic_parcels = processing_parcels(Lanatomic_parcels,pct,thr_prob,size_thr,Lparcel_names,trac,trac_path,"L")
    Ranatomic_parcels = processing_parcels(Ranatomic_parcels,pct,thr_prob,size_thr,Rparcel_names,trac,trac_path,"R")
    Lanatomic_parcels = get_hard_parcels(Lanatomic_parcels)
    Ranatomic_parcels = get_hard_parcels(Ranatomic_parcels)
    IO.write_hparcels_triangles(Lanatomic_parcels,atlas_path,"left")
    IO.write_hparcels_triangles(Ranatomic_parcels,atlas_path,"right")
    # Lparcel_cc = get_PCC(Lanatomic_parcels,Ltriangles)
    # Rparcel_cc = get_PCC(Ranatomic_parcels,Rtriangles)
    Lparcel_cc = get_PCC_2(Lanatomic_parcels,Ltriangles)
    Rparcel_cc = get_PCC_2(Ranatomic_parcels,Rtriangles)
    IO.write_cc(Lparcel_cc,cc_path,"left")
    IO.write_cc(Rparcel_cc,cc_path,"right")
    t2 = time.time()
    ...
    finish = time.time()
    print("Time to parcel the atlas: ", finish-start)
