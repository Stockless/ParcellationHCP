...
def filtering (start, end, path, min_size):
    ...
    for x in range(start,end+1):
        ...
        name= 'resampled-tractography.bundles'
        arg= ['python','filtering/filtering.py',path+sub+'/'+name, path + sub +
...
def transform_individual_clusters (start, end, path, path_matrix, theta):
    #path example: path='Archi_y_codigos/resampling/filtering/transform/ffclust/inter-subject_clustering/ARCHI/subs/'
    #path_matrix example: path_matrix='/transformmatrices/trmT1ToTal.trm'
    t0 = time.time()
    #Si fuese necesario, compilar antes:
    ...
    for x in range(start,end+1):
        ...
        arg= ['transform/./transform', path_individual , path_individual+'T1/', path + sub + path_matrix]
        call(arg)   
