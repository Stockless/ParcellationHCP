...
def preliminar_subparcels (start, end, sub_selected, intersection_path, meshes_path, parcel_names):
    ...
    for i in range(start, end+1):
        ...
        for tipo in int_type:
            int_path = intersection_path + '/' + sub + '/' + sub + '_' + tipo + '-hemi/'
            ...
            for file in int_list:
                parcel_name = file.split('.')[0].split('_')[1]
                bundle_name = get_bundle_names(file)
                ...
                if tipo == 'left':
                    if parcel1_name not in L_sp_dict:
                        L_sp_dict[parcel1_name] = {}
                    if parcel2_name not in L_sp_dict:
                        L_sp_dict[parcel2_name] = {}
                        
                elif tipo == 'right':
                    if parcel1_name not in R_sp_dict:
                        R_sp_dict[parcel1_name] = {}
                    if parcel2_name not in R_sp_dict:
                        R_sp_dict[parcel2_name] = {}
                
                #Lectura de intersección y obtención de triángulos iniciales y finales
                ix = bt.read_intersection_txt(int_path + file)
                in_tri = ix[0]
                fn_tri = ix[1]
...
def hard_parcels (start, end, sub_selected, intersection_path, meshes_path, parcel_names, dc_thr, idc_thr, output):
    #Parcelas preliminares
    Lhemi_dict, Rhemi_dict, L_sp_dict, R_sp_dict = preliminar_subparcels (start, end, sub_selected, intersection_path, meshes_path, parcel_names)
...
def final_parcels (output, meshes_path, sub_selected, ero, dil):
    ...
    print(len(Lparcels_hard),len(Rparcels_hard))
    print('OK')
    ...
    sproc.call(['make']);
    sproc.call(['./main', Lhemi_path, Rhemi_path, str(ero), str(dil),  Lparcelcc_path, Rparcelcc_path, Lfinal_path, Rfinal_path]);
...
# Lparcels_hard, Rparcels_hard = load_parcels('hard','Parcellation/QB_'+str(theta_QB)+'/IDC_DC_'+str(idc_thr*100)+'%_'+str(dc_thr*100)+'%')
# visualize_parcellation(meshes_path, Lparcels_hard, Rparcels_hard, '001', seed = semilla_visualizacion)

# Lparcel_cc, Rparcel_cc = load_parcels('cc',output_parcellation)
